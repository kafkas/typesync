...
class TypesyncUndefined:
    """Do not use this class in your code. Use the `UNSET` sentinel instead."""
    _instance = None

    def __init__(self):
        if TypesyncUndefined._instance is not None:
            raise RuntimeError("TypesyncUndefined instances cannot be created directly. Import and use the UNSET sentinel instead.")
        else:
...
UNSET = TypesyncUndefined()
"""A sentinel value that can be used to indicate that a value should be undefined. During serialization all values that are marked as undefined will be removed. The difference between `UNSET` and `None` is that values that are set to `None` will serialize to explicit null."""
...
class Profile(TypesyncModel):
    bio: typing.Union[TypesyncUndefined, str] = UNSET
    ...
    def __setattr__(self, name: str, value: typing.Any) -> None:
        if name == "bio" and value is None:
            raise ValueError("'bio' field cannot be set to None")
